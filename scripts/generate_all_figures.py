...
sns.barplot(x=cat_f1, y=models, palette='viridis', hue=models, legend=False)
...
plt.figure(figsize=(10, 6))
sns.barplot(x=pri_f1, y=models, palette='magma', hue=models, legend=False)
...
boxes = [
    ("Low Confidence\nPrediction (<0.8)", (0.2, 0.5), "lightpink"),
    ("Dashboard UI\n(Amber Badge)", (0.45, 0.5), "moccasin"),
    ("Human Agent\nCorrection", (0.7, 0.5), "lightblue"),
    ("Database\n(corrections table)", (0.95, 0.5), "lightgreen"),
    ("retrain_from_feedback.py\n(Offline Script)", (0.95, 0.15), "plum"),
    ("Deploy\ncategory_pipeline_v2.pkl", (0.45, 0.15), "lightgrey")
]

for text, pos, color in boxes:
    # Use generic rectangle for simplicity since FancyBox parameter can be tricky 
    ax.add_patch(plt.Rectangle((pos[0]-0.1, pos[1]-0.15), 0.2, 0.3, facecolor=color, edgecolor='black'))
    ax.add_patch(plt.Rectangle((pos[0]-0.1, pos[1]-0.15), 0.2, 0.3, fill=False))
    ax.text(pos[0], pos[1], text, ha='center', va='center', fontsize=10, fontweight='bold')

plt.annotate("", xy=(0.35, 0.5), xytext=(0.3, 0.5), arrowprops=dict(arrowstyle="->", lw=2, color='gray'))
plt.annotate("", xy=(0.6, 0.5), xytext=(0.55, 0.5), arrowprops=dict(arrowstyle="->", lw=2, color='gray'))
plt.annotate("", xy=(0.85, 0.5), xytext=(0.8, 0.5), arrowprops=dict(arrowstyle="->", lw=2, color='gray'))
plt.annotate("", xy=(0.95, 0.35), xytext=(0.95, 0.45), arrowprops=dict(arrowstyle="->", lw=2, color='gray'))
plt.annotate("", xy=(0.55, 0.15), xytext=(0.85, 0.15), arrowprops=dict(arrowstyle="->", lw=2, color='gray'))
